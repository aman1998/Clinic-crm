...
import { employeesService, ENTITY, hospitalService } from '../../../../services';
...
export function ListPatientHospital({ patientsUuid }) {
	...
	const { isLoading, isError, data } = useQuery([ENTITY.HOSPITAL_RECEPTION, debouncedForm], ({ queryKey }) =>
		hospitalService.getHospitalReceptions(queryKey[1])
	);

	const statusHospitalReceptionsList = hospitalService.getStatusHospitalReceptionsList();

	const [selectedHospitalReceptionUuid, setSelectedHospitalReceptionUuid] = useState(null);
	...
	const tableOptions = {
		...
		onRowClick: (_, rowMeta) => {
			...
			setSelectedHospitalReceptionUuid(uuid);
		},
	...
	return (
		<>
			...
			{selectedHospitalReceptionUuid && (
				<ModalHospitalReception
					isOpen
					hospitalReceptionUuid={selectedHospitalReceptionUuid}
					onClose={() => setSelectedHospitalReceptionUuid(null)}
				/>
			)}
		</>
	);
}
ListPatientHospital.propTypes = {
